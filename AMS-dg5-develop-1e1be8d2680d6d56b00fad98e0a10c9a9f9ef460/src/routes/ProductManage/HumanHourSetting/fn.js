...
export const onInitial = _this => async () => {
  ...
  const productData = rows.map(({ id, lineNname }) => ({ id, lineNname }));
...
export const shouldModalShowUpdate = _this => () => {
  ...
  if (selectedData) {
    _this.setState({
      ...
      workTimeA: selectedData.workTimeA,
      workTimeB: selectedData.workTimeB,
    });
  }
  return true;
};
export const onRefresh = (tools) => {
  ...
  const props = getProps('HumanHourSettingForm');
