...
import { actionDel, productAction, lineTypeAction } from './config';
...
export const onInitial = _this => async () => {
  ...
  const { message: { rows: lineData } = {} } = await fetchData(lineTypeAction);
  _this.setState({ productData, lineData });
};
...
export const shouldModalShowUpdate = _this => () => {
  ...
  if (selectedData) {
    _this.setState({
      ...
      lineType: selectedData.lineType,
      upha: selectedData.upha,
      uphb: selectedData.uphb,
    });
  }
  return true;
};
export const onRefresh = (tools) => {
  ...
  const props = getProps('UPHSettingForm');
