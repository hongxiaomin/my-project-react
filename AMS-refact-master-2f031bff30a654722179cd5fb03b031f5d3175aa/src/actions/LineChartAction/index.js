...
const lineGenerator = ({ lines }) => (
  () => {
    if (Array.isArray(lines)) {
      return lines.map(line => <Line key={line.dataKey} {...line} />);
    }
    return undefined;
  }
);
...
export const onLineChartLoad = ({ name, action, onSuccess, onError }) => (
  (dispatch) => {
    const url = action;
    const method = GET;
    ...
    const error = (e) => {
      dispatch(onLineChartLoadFail(e));
      Error(e);
    };
    Request({
      url,
...
export const onLineChartInitial = (id, props) => (
  (dispatch) => {
    let children = [];
...
export const onLineChartRefreshData = (oriChildren, oriData, props) => (
  (dispatch) => {
