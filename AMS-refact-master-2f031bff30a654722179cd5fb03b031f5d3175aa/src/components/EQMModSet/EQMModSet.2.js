...
const targetKeyUpdate = ['eqpModelName', 'eqpModelCode', 'eqpModelDesc', 'eqpTypeId', 'commModeId', 'lastUpdateBy', 'id'];

const readImg = function (id) {
  return new Promise((resolve, reject) => {
    const error = (e) => {
      console.error(e);
    };
    const callback = (res) => {
      localStorage.setItem(id, res.rows);
      resolve();
    };
    const url = `${SERVER_IP_EQM}/ams/eqm/eqp/model/picture?id=${id}`;
    Request({
      url,
      callback,
      error,
...
const reRenderFunc = function* (id) {
  // 若已有缓存，则不发送请求
...
const columns = [
  {
    ...
    render: (text, record, index) => {
      if (record.id) {
        reRenderFunc(record.id);
        return <img key={index} src={localStorage.getItem(record.id)} />;
      }
    },
  }, {
    title: '型号描述',
