...
export let graphXmlParseData = {};
export let graphXmlupdateParseData = {};
...
// 画布有操作，执行函数
    ...
    // 右键执行编辑、删除操作
          ...
          if (model.isEdge(cell)) {
            ...
            menu.addItem('PASS', null, () => {
              ...
              if (xml) {
                ...
                graphXmlParseData = updateData;
                graphXmlupdateParseData = updateData;
              }
      ...
      if (xml) {
        ...
        graphXmlParseData = addData;
        graphXmlupdateParseData = addData;
      }
...
// 左侧栏
const addVertexInit = (addVertextFunc, stationListData, routingListData, toolbarName, toolBarDiv, startEnd) => {
  if (toolbarName && toolBarDiv) {
    ...
    if (stationListData.length > 0) {
      ...
      toolBarNameTitle.setAttribute('class', 'title');
      ...
      toolBarBoxTitle.setAttribute('class', 'title');
      toolBarBoxTitle.innerHTML = '站别';
      ...
      toolbar.enabled = false;
      stationListData.map((v, i) => {
        const div = document.createElement('div');
        div.innerHTML = `${v.name}`;
        const width = v.name.length * 7;
        if (width > 80) {
          addVertextFunc(v.name, rectangle, width, 30, `nodeId=${v.id};type=STATION;`, v.name);
        } else {
          addVertextFunc(v.name, rectangle, 80, 30, `nodeId=${v.id};type=STATION;`, v.name);
        }
        if (toolbarName) {
    ...
    if (routingListData.length > 0) {
      const toolBarNameBox = document.createElement('div');
      toolBarNameBox.setAttribute('class', 'toolBarroutingNameBox');
      toolBarNameBox.setAttribute('id', 'toolBarroutingNameBox');
      const toolBarRoutingNameTitle = document.createElement('a');
      // toolBarRoutingNameTitle.setAttribute('class', 'title');
      toolBarRoutingNameTitle.innerHTML = '<strong>∨</strong>';
      toolBarNameBox.appendChild(toolBarRoutingNameTitle);
      const toolBarNameContent = document.createElement('div');
      toolBarNameBox.appendChild(toolBarNameContent);
      const toolBarBox = document.createElement('div');
      const toolBarRoutingBoxTitle = document.createElement('a');
      toolBarRoutingBoxTitle.innerHTML = '途程';
      toolBarBox.appendChild(toolBarRoutingBoxTitle);
      const toolBarContent = document.createElement('div');
      toolBarContent.setAttribute('id', 'toolBarRoutingContent');
      toolBarBox.appendChild(toolBarContent);
      toolBarDiv.appendChild(toolBarBox);

      toolbar = new mxgraph.mxToolbar(toolBarContent);
      toolbar.enabled = false;
      routingListData.map((v, i) => {
        const div = document.createElement('div');
        div.innerHTML = `${v.name}`;
        const width = v.name.length * 7;
        if (width > 80) {
          addVertextFunc(v.name, rectangle, width, 30, `nodeId=${v.id};type=ROUTE;fillColor=#66aef4;strokeColor=#4a96e1`, v.name);
        } else {
          addVertextFunc(v.name, rectangle, 80, 30, `nodeId=${v.id};type=ROUTE;fillColor=#66aef4;strokeColor=#4a96e1`, v.name);
        }
        if (toolbarName) {
          toolBarNameContent.appendChild(div);
        }
      ...
      toolbarName.appendChild(toolBarNameBox);
      const barBox = document.getElementById('toolBarRoutingContent');
      const nameBox = document.getElementById('toolBarroutingNameBox');
      nameBox.lastElementChild.style.display = 'none';
      barBox.style.display = 'none';
      toolBarRoutingBoxTitle.addEventListener('click', () => {
        routingTitleClick(toolBarRoutingNameTitle, barBox, nameBox);
      }, false);
      toolBarRoutingNameTitle.addEventListener('click', () => {
        routingTitleClick(toolBarRoutingNameTitle, barBox, nameBox);
      }, false);
    }
  }
};
const showDataList = (stationShowState, routeShowState) => {
  const routeBarBox = document.getElementById('toolBarRoutingContent');
  const routeNameBox = document.getElementById('toolBarroutingNameBox');
  const barBox = document.getElementById('toolBarContent');
  const nameBox = document.getElementById('toolBarNameBox');
  if (barBox && nameBox) {
    nameBox.lastElementChild.style.display = stationShowState;
    barBox.style.display = stationShowState;
  }
  if (routeBarBox && routeNameBox) {
    routeNameBox.lastElementChild.style.display = routeShowState;
    routeBarBox.style.display = routeShowState;
  }
};
...
const searchDataFunc = (e) => {
  ...
  let url = `${SERVER_IP_SMT}/ams/routing/select?type=1`;
  isInput = false;
  if (name !== '') {
    url = `${SERVER_IP_SMT}/ams/routing/select?name=${name}&type=1`;
    isInput = true;
  }
  const callback = (response) => {
    if (response.code === 0) {
      ...
      if (name === '') {
        addVertexInit(addVertex, searchData.station, searchData.route, document.getElementById('toobarName'), document.getElementById('toolbar'), true);
      } else {
        addVertexInit(addVertex, searchData.station, searchData.route, document.getElementById('toobarName'), document.getElementById('toolbar'), false);
      }
      showDataList('block', 'block');
    }
  };
  Request({
    url,
...
const graphFunc = (graphData, toolbarData) => {
  ...
  const stationBox = document.getElementById('toolBarContent');
  const routeBox = document.getElementById('toolBarRoutingContent');
  let stationShowState = 'none';
  let routeShowState = 'none';
  if (stationBox) {
    stationShowState = stationBox.style.display;
  }
  if (routeBox) {
    routeShowState = routeBox.style.display;
  }
  if (isSearch) {
    if (isInput) {
      addVertexInit(addVertex, searchToolBarData.station, searchToolBarData.route, document.getElementById('toobarName'), document.getElementById('toolbar'), false);
    } else {
      addVertexInit(addVertex, searchToolBarData.station, searchToolBarData.route, document.getElementById('toobarName'), document.getElementById('toolbar'), true);
    }
  } else {
    addVertexInit(addVertex, toolbarData.station, toolbarData.route, document.getElementById('toobarName'), document.getElementById('toolbar'), true);
  }
  showDataList(stationShowState, routeShowState);
};
