...
import styles from './rechartRadialBarEditorModalStyles';
...
export default class ReChartRadialBarEditorModal extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const {
      url, transformer, legend, dataKey, color, outerRadius, innerRadius,
    } = this.state.dataTransformer;
            ...
            <div style={{ display: 'flex', flex: 1 }}>
              <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 50 }}>
                ...
                <div style={{ marginTop: 10, display: 'flex' }}>
                  ...
                  />
                  <DefaultTextField floatingLabelText="innerRadius"
                    value={innerRadius} onChange={this.onChange('innerRadius')}
                  />
                </div>
              </div>
              <div style={{ flex: 1, marginLeft: 65 }}>
                <div style={{ marginTop: 5, display: 'flex' }}>
                  <DefaultTextField floatingLabelText="dataKey"
                    value={dataKey} onChange={this.onChange('dataKey')}
                    errorText={isEmpty(dataKey).hint}
                    errorStyle={isEmpty(dataKey).style}
                  />
                  ...
                  <ColorPickerTextField
                    TextField={
                      <DefaultTextField floatingLabelText="Color" value={color}
                        onChange={this.onChange('color')} style={{ width: 150 }}
                      />
                    }
                    colorValue={color}
...
ReChartRadialBarEditorModal.propTypes = {
